    ...
    */
    ...
    public R save(@RequestBody QicheEntity qiche, HttpServletRequest request){
        ...
        Wrapper<QicheEntity> queryWrapper = new EntityWrapper<QicheEntity>()
            .eq("qiche_uuid_number", qiche.getQicheUuidNumber())
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            QicheEntity qicheEntity = new QicheEntity();
//                            qicheEntity.setQicheUuidNumber(data.get(0));                    //汽车编号 要改的
...
//                            qicheEntity.setQicheKucunNumber(Integer.valueOf(data.get(0)));   //汽车库存数量 要改的
//                            qicheEntity.setQicheOldMoney(data.get(0));                    //租赁原价/天 要改的
//                            qicheEntity.setQicheNewMoney(data.get(0));                    //现价/天 要改的
//                            qicheEntity.setQicheClicknum(Integer.valueOf(data.get(0)));   //热度 要改的
...
//                            qicheEntity.setCreateTime(date);//时间
                            ...
                            //把要查询是否重复的字段放入map中
                                //汽车编号
                                if(seachFields.containsKey("qicheUuidNumber")){
                                    List<String> qicheUuidNumber = seachFields.get("qicheUuidNumber");
                                    qicheUuidNumber.add(data.get(0));//要改的
                                }else{
                                    List<String> qicheUuidNumber = new ArrayList<>();
                                    qicheUuidNumber.add(data.get(0));//要改的
                                    seachFields.put("qicheUuidNumber",qicheUuidNumber);
                                }
                        }

                        //查询是否重复
                         //汽车编号
                        List<QicheEntity> qicheEntities_qicheUuidNumber = qicheService.selectList(new EntityWrapper<QicheEntity>().in("qiche_uuid_number", seachFields.get("qicheUuidNumber")).eq("qiche_delete", 1));
                        if(qicheEntities_qicheUuidNumber.size() >0 ){
                            ArrayList<String> repeatFields = new ArrayList<>();
                            for(QicheEntity s:qicheEntities_qicheUuidNumber){
                                repeatFields.add(s.getQicheUuidNumber());
                            }
                            return R.error(511,"数据库的该表中的 [汽车编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
